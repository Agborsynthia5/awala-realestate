...
class _EditListingScreenState extends ConsumerState<EditListingScreen> {
  ...
  void initState() {
    super.initState();
    _uploadedImages.addAll(widget.property.images);
    _selectedAmenities.addAll(widget.property.amenities);
    _selectedLatLng = LatLng(
      widget.property.latitude ?? AppConstants.molykoLat,
      widget.property.longitude ?? AppConstants.molykoLng,
    );
  }
  ...
  Future<void> _submitListing() async {
    ...
    try {
      ...
      await ref.read(apiServiceProvider).updateProperty(widget.property.id, payload);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to update listing: $e'), backgroundColor: AppColors.error),
        );
      }
    }
  }
  ...
  Widget _buildStep2Details() {
    return FormBuilder(
      ...
      initialValue: {
        ...
        'neighborhood': widget.property.neighborhood,
        'location_name': widget.property.locationName,
      },
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
